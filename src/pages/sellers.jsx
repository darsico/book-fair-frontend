...
const Sellers = () => {
 const { data, isLoading, error } = useQuery('seller', getSellers);
 const [params, setParams] = useSearchParams()
 if (isLoading) return <SpinnerSection />;
 if (error) return <h1>Something went wrong</h1>;
 if (data?.length === 0) return <h1>No sellers yet</h1>
 const { sellers } = data;
   ...
   <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4  gap-10">
    {filteredSellers.map((seller) => (
     <div key={seller._id} className="p-2 border border-solid " >
      ...
      <p className="text-sm ">Owner:<span className="text-gray-600"> {seller.name}</span></p>
      <p className="text-sm ">Email: {seller.email}</p>
      <p>{seller.books.length >= 1 ? `${seller.books.length} books` : "No books yet"}</p>
      <Link to={`/sellers/${seller._id}`}><button className="text-lg mt-4 underline">Go to store</button></Link>
     </div>
    ))}
   </div>
  </section>
